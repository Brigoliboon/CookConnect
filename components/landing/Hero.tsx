...
export function Hero() {
  return (
    <section className="relative min-h-[122vh] overflow-hidden max-h-[122vh] bg-black">
      <div
        className="absolute h-[122vh] inset-0 bg-cover bg-center opacity-40"
        style={{ backgroundImage: "url(/bg.jpg)" }}
      />
      <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-black/40 to-black/90" />

      <div className="relative z-10 flex min-h-screen flex-col justify-between px-8 pt-28">
        <motion.div
          variants={container}
          initial="hidden"
          animate="show"
          className="grid items-end gap-16 lg:grid-cols-2"
        >
          <div>
            <motion.p variants={fadeUp} className="text-[11px] font-semibold uppercase tracking-[0.3em] text-white/40">
              Subscription Meal Plans
            </motion.p>
            <motion.h1 variants={fadeUp} className="font-playfair mt-5 text-5xl font-medium leading-tight text-white sm:text-6xl lg:text-7xl">
              Fresh, Custom
              <br />
              Meal Plans —
              <br />
            ...
            <motion.div variants={fadeUp} className="mt-10 flex items-center gap-4">
              <Link
                href="#subscription"
                className="rounded-xl bg-white px-7 py-3 text-sm font-semibold text-black transition-all hover:bg-white/90"
              >
                View Subscriptions
              </Link>
              <Link
                href="#meals"
                className="rounded-xl border border-white/20 px-7 py-3 text-sm font-semibold text-white transition-all hover:bg-white/10"
              >
                Explore Menu
              </Link>
            </motion.div>
          ...
          <motion.div variants={fadeUp} className="flex items-end justify-end">
            <img className="w-full max-w-lg object-contain" src="/hero-menu.png" alt="Menu preview" />
          </motion.div>
        </motion.div>
        ...
        <motion.div
          ...
          className="flex gap-10 overflow-x-auto pb-2 justify-center overflow-hidden">
            {featured.map((item) => (
